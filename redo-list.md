...
| [Recursive palindrome check](weeks/week-01-recursion-fundamentals/notes/palindrome_check.md) | 1 | Base case took three attempts. `r < l` returned `false`, so every even-length palindrome failed; the second attempt read `s[l-1]` and went out of bounds on `""`. | ☐ | ☐ |
